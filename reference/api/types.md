<!-- vendored from https://api.tabletopsimulator.com/types/ — do not edit, regenerate with scripts/fetch_api_docs.py -->
...
| *bool* | bool | `true` or `false` value. | `true` |
...
| *func* | function | See section [Function](#function) for more info. |  |
| *object* | object | An in-game physical Object. Sometimes Global. | `Globalorself` |
| *player* | player | An in-game Player. See [Player](player__instance.md) for more info. | `Player["White"]` |
| *var* | variable | A combination of other types. |  |
...
You are also able to use a String in place of a color table. Using a [Player Color](player__colors.md) will automatically fill in that value. It works with any of the 12 color names, as they are written on the [Player Color](player__colors.md) page.
...
*func*  A function is a section of code that can be run when triggered. In Lua, you are able to pass (use as a parameter) functions. Some elements in Tabletop Simulator can be passed functions to perform some action with, like triggering it after a delay.
